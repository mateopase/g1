...
public class SpatialHash<T>
{
    ...
    public void UpdatePosition(Vector2 vector, T obj)
    {
    ...
    public List<T> QueryPosition(Vector2 vector)
    {
        var key = Key(vector);
    ...
    public List<T> QueryRelativePosition(Vector2 vector, 
                                int cellsLeft, int cellsUp, int cellsForward)
    {
        ...
        return QueryPosition(vector + (posLeft * Vector2.left) + 
                                    (posUp * Vector2.up) +
                                    (posForward * Vector2.forward) );
    }
}